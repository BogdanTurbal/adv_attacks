...
# Execute the main experiment steps (from usefatt directory, which contains run_experiment_unified.sh)
...
def submit_parallel_jobs(subexp_name: str, subexp_config: Dict[str, Any], global_config: Dict[str, Any], dry_run: bool = False) -> List[str]:
    ...
    if total_examples_in_range > 0:
        ...
        # Create results directory (inside reasoning_attacks_res, will save CSV after job submission with SLURM IDs)
        base_results_dir = f"/scratch/gpfs/KOROLOVA/bt4811/usefatt/reasoning_attacks_res/{results_dir}"
    ...
    if total_examples_in_range > 0:
        # Create results directory if not already created (inside reasoning_attacks_res)
        base_results_dir = f"/scratch/gpfs/KOROLOVA/bt4811/usefatt/reasoning_attacks_res/{results_dir}"
        os.makedirs(base_results_dir, exist_ok=True)
        assignment_csv_path = os.path.join(base_results_dir, f"{subexp_name}_assignment.csv")
        ...
        with open(assignment_csv_path, 'w', newline='') as f:
            ...
            for job_id, gpu_id, abs_start, abs_end in assignments_with_offset:
                ...
                # Results are saved to: base_results_dir/job_id (e.g., /scratch/gpfs/KOROLOVA/bt4811/usefatt/reasoning_attacks_res/exp_20_results_ort_beta_0_5/1)
