...
INPUT_CSV=${INPUT_CSV:-"dataset/orthogonalized_outputs_cot150_2048.csv"}
...
if [ -n "$RESULTS_DIR_PARAM" ]; then
    BASE_RESULTS_DIR="/scratch/gpfs/KOROLOVA/bt4811/b2/$RESULTS_DIR_PARAM"
else
    BASE_RESULTS_DIR="/scratch/gpfs/KOROLOVA/bt4811/b2/results_default"
fi
...
python -c "import wandb; wandb.init(project='$WANDB_PROJECT', entity='$WANDB_ENTITY', dir='$WANDB_RUN_DIR')"
...
python -m attack.run_experiments \
    --beta $BETA \
    --num-steps $NUM_STEPS \
