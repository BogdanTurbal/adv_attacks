...
hf_cache = f"/scratch/gpfs/KOROLOVA/huggingface"
...
RESULTS_DIR = "/scratch/gpfs/KOROLOVA/bt4811/results/"
...
def load_input_data(csv_path: str, max_examples: int = -1, example_range: str = None) -> pd.DataFrame | None:
    ...
    try:
        ...
        if example_range:
            ...
            if len(df) > 0:
                actual_start_idx = df.index[0]
                actual_end_idx = df.index[-1]
                print(f"[EXAMPLE RANGE] Processing examples {start_idx}-{end_idx-1} ({len(df)} examples) from {csv_path}")
                print(f"[EXAMPLE RANGE] Actual dataset indices: {actual_start_idx} to {actual_end_idx} (inclusive)")
            else:
                print(f"[EXAMPLE RANGE] No examples in range {start_idx}-{end_idx-1} from {csv_path}")
        elif max_examples == 0:
...
def run_single_experiment(args: Tuple) -> Dict:
    ...
    try:
        ...
        print(
            f"[EXAMPLE ID {prompt_idx}] GPU {_worker_gpu_id}: Starting experiment with beta={beta}"
        )
        if use_override:
        ...
        print(f"[EXAMPLE ID {prompt_idx}] GCG run took {gcg_run_time:.3f}s, Total experiment: {runtime:.3f}s")
        ...
        print(
            f"[EXAMPLE ID {prompt_idx}] GPU {_worker_gpu_id}: Completed experiment in {runtime:.1f}s"
        )
...
def main():
    ...
    try:
        ...
        # Check actual GPU availability and adjust if necessary
        if torch.cuda.is_available():
            available_gpus = torch.cuda.device_count()
            if args.num_gpus > available_gpus:
                print(f"⚠️  Warning: Requested {args.num_gpus} GPUs but only {available_gpus} are available.")
                print(f"   Adjusting to use {available_gpus} GPUs instead.")
                args.num_gpus = available_gpus
            else:
                print(f"✓ CUDA available: {available_gpus} GPU(s) detected, using {args.num_gpus}")
        else:
            raise RuntimeError("CUDA is not available. Cannot run experiments without GPU support.")
