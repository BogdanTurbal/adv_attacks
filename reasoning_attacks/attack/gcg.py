    ...
    def _compute_candidates_loss_original(
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                # --- Simplified DynamicCache Handling ---
                if self.prefix_cache:
                    # Always create a fresh expanded cache for each batch to avoid memory accumulation
                    # The .expand() operation is lightweight (creates views, no copying)
                    prefix_cache_batch = DynamicCache()
                    for layer_idx in range(len(self.prefix_cache)):
                        key_states, value_states = self.prefix_cache[layer_idx]
                        # Use .expand() which is memory-efficient as it creates a view without copying data.
                ...
                if additional_tokens_needed > 0:
                    ...
                    # The final pass to get logits for the full extended sequence
                    outputs = self.model(
                        inputs_embeds=extended_embeds,
                        past_key_values=prefix_cache_batch,
                        use_cache=False,
                    ...
                    del current_kv_cache # Clean up intermediate cache
                else:
                ...
                # Minimal cleanup inside the tight loop
                del outputs, logits, shift_logits, batch_loss, final_embeds
                if prefix_cache_batch is not None:
                    del prefix_cache_batch
                
                # Clear layer activations to prevent memory accumulation
                self.layer_activations.clear()

        # More thorough cleanup can be done outside the loop if needed
        gc.collect()
        torch.cuda.empty_cache()
