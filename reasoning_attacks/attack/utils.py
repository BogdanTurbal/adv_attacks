...
def find_executable_batch_size(  # typing: ignore
    ...
    def decorator(*args, **kwargs):
        ...
        batch_size_start_time = time.time()
        gc.collect()
        ...
        while True:
            ...
            try:
                ...
                total_batch_time = time.time() - batch_size_start_time
                
                logger.info(f"Batch size {batch_size} succeeded after {attempts} attempts")
                return result
                ...
                if should_reduce_batch_size(e):
                    ...
                    torch.cuda.empty_cache()
                    batch_size //= 2 
                    logger.info(f"Decreasing batch size to: {batch_size} (attempt {attempts})")
                else:
                    raise
...
# ):
#     """
#     Simplified version that always returns batch size 128 for debugging.
#     """
#     if function is None:
#         return functools.partial(
#             find_executable_batch_size, starting_batch_size=starting_batch_size
...
#     def decorator(*args, **kwargs):
#         # Always use batch size 128
#         batch_size: int = 128
#         logger.info(f"Using fixed batch size: {batch_size}")
        
#         # Call the function with the fixed batch size
#         result = function(batch_size, *args, **kwargs)
#         return result
