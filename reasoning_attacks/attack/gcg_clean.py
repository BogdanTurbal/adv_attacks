...
class GCG:
    ...
    def __init__(
        ...
        self._cached_prefix_cache_batch = None  # Cache for expanded prefix cache
        self._cached_batch_size = None  # Track batch size for cache invalidation
    ...
    def run(
        ...
        if self.using_wandb:
            # Generate original response (before attack) for comparison
            original_response: str = self.generate_response(self.config.prompt_string)
    ...
    #                 # Handle DynamicCache batch expansion with caching
    #                 if (
    #                     not prefix_cache_batch
    #                     or current_batch_size != search_batch_size
    #                     or self._cached_batch_size != current_batch_size
    #                 ):
    #                     print("Creating new expanded cache")
    #                     # Check if we can reuse cached expanded cache
    #                     if (
    #                         self._cached_prefix_cache_batch is not None
    #                         and self._cached_batch_size == current_batch_size
    #                     ):
    #                         print("Reusing cached expanded cache")
    #                         prefix_cache_batch = self._cached_prefix_cache_batch
    #                     else:
    #                         print("Creating new expanded cache")
    #                         # Create new expanded cache
    #                         if isinstance(self.prefix_cache, DynamicCache):
    #                             print("Creating new DynamicCache for the batch")
    #                             # Create a new DynamicCache for the batch
    #                             prefix_cache_batch = DynamicCache()
    #                             print(f"{len(self.prefix_cache)} layers in prefix cache")
    #                             for layer_idx in range(len(self.prefix_cache)):
    #                                 # DynamicCache returns (key_states, value_states) tuple when indexed
    #                                 key_states, value_states = self.prefix_cache[layer_idx]
    ...
    #                         # Cache the expanded prefix cache for reuse
    #                         self._cached_prefix_cache_batch = prefix_cache_batch
    #                         self._cached_batch_size = current_batch_size
    ...
    #                     print(f"Generating additional tokens {additional_tokens_needed}")
    #                     for _ in range(additional_tokens_needed):
    #                         # Get logits for the last position
    ...
    #                     # Clean up the growing current_kv_cache to prevent memory leak
    ...
    #                 print(f"Time took for additional tokens generation: {time.time() - start_time}s")
    #             else:
    #                 # Non-prefix case remains largely the same
    #                 print("Using non-prefix cache")
    #                 if additional_tokens_needed > 0:
    #                     extended_embeds: Tensor = input_embeds_batch.clone()
    ...
    #             del outputs, logits, shift_logits, batch_loss
                
    #             # Only perform memory cleanup every 5 batches to reduce overhead
    #             if i % 5 == 0:
    #                 gc.collect()
    #                 torch.cuda.empty_cache()
    ...
    def _compute_candidates_loss_original(
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                ...
                # --- Memory-efficient DynamicCache Handling ---
                # Don't cache expanded cache to save memory (recreate each time is still fast with .expand())
                if self.prefix_cache:
                    prefix_cache_batch = DynamicCache()
                    for layer_idx in range(len(self.prefix_cache)):
                        key_states, value_states = self.prefix_cache[layer_idx]
                        # Use .expand() which creates views (no data copying) - very memory efficient
                        ...
                        prefix_cache_batch.update(
                            key_states_expanded, value_states_expanded, layer_idx
                        )
                # ----------------------------------------
                ...
                final_embeds: Tensor
                if additional_tokens_needed > 0:
                    # Logic for generating additional tokens (autgressively)
                    outputs = self.model(
                        inputs_embeds=input_embeds_batch,
                        past_key_values=prefix_cache_batch,
                        use_cache=True,
                    )
                    extended_embeds = input_embeds_batch
                    ...
                    # Final pass: use cache to compute efficiently, but free intermediate cache
                    # Free the growing current_kv_cache to save memory before final pass
                    del current_kv_cache
                    # prefix_cache_batch caches the prefix, saving computation
                    outputs = self.model(
                        inputs_embeds=extended_embeds,
                        past_key_values=prefix_cache_batch,
                        use_cache=True,  # True to USE the cache (but output cache will be freed)
                    )
                    # Explicitly free the output cache to save memory
                    del outputs.past_key_values
                    final_embeds = extended_embeds
                else:
                    # Standard forward pass without extra generation
                    # Use cache to speed up, but free output cache immediately
                    outputs = self.model(
                        inputs_embeds=input_embeds_batch,
                        past_key_values=prefix_cache_batch,
                        use_cache=True,  # True to USE the cache
                    )
                    # Explicitly free the output cache to save memory
                    del outputs.past_key_values
                ...
                original_input_length: int = input_embeds_batch.shape[1]
                tmp: int = original_input_length - target_length
                
                shift_logits: Tensor = logits[..., tmp - 1 : tmp - 1 + target_length, :].contiguous()
                shift_labels: Tensor = self.target_ids.repeat(current_batch_size, 1)
                ...
                if self.config.early_stop and torch.all(torch.argmax(shift_logits, dim=-1) == shift_labels, dim=-1).any():
                    self.stop_flag = True
