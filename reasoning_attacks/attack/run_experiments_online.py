...
Online version that downloads files on the fly.
"""
...
try:
    ...
    print(f"Error importing attack module: {e}")
    print("This might be due to transformers version compatibility issues.")
    print("Please ensure you're using a compatible version of transformers.")
    raise

warnings.filterwarnings("ignore", message="Flash Attention defaults to a non-deterministic algorithm")

# Set HF cache location - use a local cache directory for online downloads
hf_cache = os.path.expanduser("~/.cache/huggingface")
os.environ["HF_HOME"] = hf_cache
os.environ["HUGGINGFACE_HUB_CACHE"] = hf_cache

# Enable online mode for HuggingFace (remove offline settings)
# os.environ["HF_HUB_OFFLINE"] = "1"  # Commented out for online mode
# os.environ["TRANSFORMERS_OFFLINE"] = "1"  # Commented out for online mode
...
def check_transformers_compatibility():
    """Check if transformers version is compatible with the attack code."""
    try:
        import transformers
        ...
        if major >= 4 and minor >= 44:
            print("⚠️  Warning: You're using a newer version of transformers that may have compatibility issues.")
            print("   The DynamicCache API has changed in recent versions.")
            print("   Consider using transformers==4.43.0 or earlier for best compatibility.")
...
def ensure_model_downloaded(model_name: str, cache_dir: str):
    """Ensure the model is downloaded and available locally."""
    print(f"Checking if {model_name} is available locally...")
...
def ensure_dataset_downloaded(csv_path: str):
    """Ensure the dataset CSV file exists, download if needed."""
    if os.path.exists(csv_path):
        print(f"✓ Dataset {csv_path} already available locally")
        return True
    
    print(f"Dataset {csv_path} not found locally")
    print("Please ensure the dataset file is available or update the path")
    return False


def apply_dynamic_cache_patch():
    """Apply a patch to fix DynamicCache compatibility issues."""
    try:
        import transformers
        ...
        # Apply patch for newer versions
        ...
        print(f"⚠️  Could not apply DynamicCache patch: {e}")
        return False
...
def worker_init(gpu_id: int):
    ...
    try:
        ...
        print("Applying DynamicCache compatibility patch...")
        apply_dynamic_cache_patch()
        
        # Set CUDA device - this is the key fix!
        # Instead of setting CUDA_VISIBLE_DEVICES, directly set the device
        ...
        if not ensure_model_downloaded(MODEL_NAME, hf_cache):
        ...
        _worker_model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map=f"cuda:{gpu_id}",  # Use the actual GPU ID
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help=f"Wandb entity name (default: {WANDB_ENTITY})",
    )
    parser.add_argument(
        "--wandb-project",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Maximum number of examples to process (-1 for unlimited, 0 for none)",
    )
    parser.add_argument(
        "--example-range",
        type=str,
        default=None,
        help="Example range in format 'start:end' (integers) or 'start_pct:end_pct' (percentages 0.0-1.0). Examples: '0:100', '0.25:0.5'",
    )
    return parser.parse_args()


def parse_example_range(range_str: str, total_examples: int) -> Tuple[int, int]:
...
def load_input_data(csv_path: str, max_examples: int = -1, example_range: str = None) -> pd.DataFrame | None:
    """Load input data from CSV file with optional range selection."""
    try:
        # Ensure dataset is available
        if not ensure_dataset_downloaded(csv_path):
        ...
        # Apply example range selection if specified
        if example_range:
            start_idx, end_idx = parse_example_range(example_range, original_count)
            df = df.iloc[start_idx:end_idx]
            print(f"Selected examples {start_idx}-{end_idx} ({len(df)} examples) from {csv_path}")
        elif max_examples == 0:
            print(f"No examples to process (max_examples=0)")
            return pd.DataFrame(columns=required_columns)
        elif max_examples > 0:
            df = df.head(max_examples)
            print(f"Loaded {len(df)} experiments from {csv_path} (limited from {original_count})")
        else:
...
def monitor_memory(stage: str):
    ...
    if torch.cuda.is_available():
        allocated = torch.cuda.memory_allocated(_worker_gpu_id) / 1e9
        reserved = torch.cuda.memory_reserved(_worker_gpu_id) / 1e9
        print(
...
def run_single_experiment(args: Tuple) -> Dict:
    ...
    try:
        ...
        gcg_run_time = time.time() - gcg_run_start_time

        end_time = time.time()
        runtime = end_time - start_time
        
        print(f"Experiment {prompt_idx}: GCG run took {gcg_run_time:.3f}s, Total experiment: {runtime:.3f}s")
...
def main():
    ...
    try:
        # Check transformers compatibility first
        print("Checking transformers compatibility...")
        if not check_transformers_compatibility():
            print("⚠️  Compatibility check failed, but continuing...")
        
        # Apply DynamicCache patch globally
        print("Applying DynamicCache compatibility patch...")
        apply_dynamic_cache_patch()
        
        args = parse_args()

        print("\n=== Running GCG Experiments (Online Mode) ===")
        ...
        print(f"Creating results directory: {args.results_dir}")
        os.makedirs(args.results_dir, exist_ok=True)
        print("✓ Results directory created")

        # Ensure cache directory exists
        print(f"Creating HuggingFace cache directory: {hf_cache}")
        os.makedirs(hf_cache, exist_ok=True)
        print("✓ Cache directory created")

        # Load and preprocess data
        print("Loading input data...")
        input_df = load_input_data(args.input_csv, args.max_examples, args.example_range)
        ...
        print("Distributing experiments across GPUs...")
        ...
        with mp.Pool(processes=args.num_gpus) as pool:
            ...
            print(f"Running experiments on {len(gpu_experiment_pairs)} GPUs...")
